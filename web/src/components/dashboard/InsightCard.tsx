...
function getStyle(impact: number, type: string): SStyle {
  if (impact > 80) return { Icon: AlertTriangle, color: "#f43f5e", bg: "rgba(244,63,94,0.10)", border: "rgba(244,63,94,0.20)", label: "Critical" };
  if (impact > 60) return { Icon: Zap, color: "#f59e0b", bg: "rgba(245,158,11,0.10)", border: "rgba(245,158,11,0.20)", label: "High" };
  if (type?.toLowerCase().includes("success") || impact < 30) return { Icon: CheckCircle, color: "#10b981", bg: "rgba(16,185,129,0.10)", border: "rgba(16,185,129,0.15)", label: "Resolved" };
  return { Icon: Info, color: "#0ea5e9", bg: "rgba(14,165,233,0.10)", border: "rgba(14,165,233,0.15)", label: "Advisory" };
}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              <span style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.07em", color: "rgba(255,255,255,0.42)" }}>Confidence</span>
          ...
          <div style={{ textAlign: "right" }}>
            <span style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.07em", color: "rgba(255,255,255,0.42)", display: "block", marginBottom: 2 }}>Impact</span>
