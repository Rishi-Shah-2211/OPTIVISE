...
function getStyle(impact: number, type: string): SStyle {
  if (impact > 80) return { Icon: AlertTriangle, color: "#e11d48", bg: "rgba(225,29,72,0.08)", border: "rgba(225,29,72,0.2)", label: "Critical" };
  if (impact > 60) return { Icon: Zap, color: "#d97706", bg: "rgba(217,119,6,0.08)", border: "rgba(217,119,6,0.2)", label: "High" };
  if (type?.toLowerCase().includes("success") || impact < 30) return { Icon: CheckCircle, color: "#059669", bg: "rgba(5,150,105,0.08)", border: "rgba(5,150,105,0.15)", label: "Resolved" };
  return { Icon: Info, color: "#0284c7", bg: "rgba(2,132,199,0.07)", border: "rgba(2,132,199,0.15)", label: "Advisory" };
}
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5, flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: 10, color: "#9CA3AF", marginLeft: "auto" }}>{safeAgo(insight.createdAt)}</span>
        </div>

        <p style={{ fontSize: 13, color: "#374151", lineHeight: 1.5, marginBottom: 10 }}>{insight.message}</p>

        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              <span style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.07em", color: "#9CA3AF" }}>Confidence</span>
          ...
          <div style={{ textAlign: "right" }}>
            <span style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.07em", color: "#9CA3AF", display: "block", marginBottom: 2 }}>Impact</span>
