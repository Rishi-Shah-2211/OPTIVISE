...
const RISK_STYLES = {
  critical: { color: "#c0492f", bg: "rgba(192,73,47,0.10)", border: "rgba(192,73,47,0.2)", label: "Act Now"     },
  high:     { color: "#c0492f", bg: "rgba(192,73,47,0.08)", border: "rgba(192,73,47,0.15)", label: "Order Soon"  },
  medium:   { color: "#c86a33", bg: "rgba(200,106,51,0.10)", border: "rgba(200,106,51,0.18)", label: "Keep Watch" },
  low:      { color: "#1f7a5c", bg: "rgba(31,122,92,0.10)", border: "rgba(31,122,92,0.15)", label: "All Good"   },
};
  ...
  return (
    <div className={`${glassCard} p-[16px_18px]`}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <span style={{ fontSize: 13, fontWeight: 500, color: "rgba(50,64,54,0.7)" }}>{label}</span>
      ...
      <input
        ...
        style={{
          ...
          background: `linear-gradient(to right, ${color} 0%, ${color} ${pct}%, rgba(62,70,54,0.14) ${pct}%, rgba(62,70,54,0.14) 100%)`,
        }}
      />
      <style>{`
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
        <span style={{ fontSize: 10, color: "rgba(50,64,54,0.48)" }}>{format ? format(min) : min}</span>
        <span style={{ fontSize: 10, color: "rgba(50,64,54,0.48)" }}>{format ? format(max) : max}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      <div style={{
        ...
        <div style={{ width: 28, height: 28, borderRadius: 10, background: "rgba(21,79,61,0.12)", border: "1px solid rgba(21,79,61,0.25)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <FlaskConical size={13} style={{ color: "#154f3d" }} strokeWidth={2} />
        </div>
        <div>
          <h1 style={{ fontFamily: "var(--font-fraunces), ui-serif, serif", fontSize: 15, fontWeight: 700, color: "#1b1d1b", margin: 0 }}>What-If Tool</h1>
          <p style={{ fontSize: 11, color: "rgba(50,64,54,0.60)", margin: 0 }}>See what happens before you buy</p>
        </div>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "28px 32px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "360px 1fr", gap: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div>
              <p style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "rgba(50,64,54,0.48)", marginBottom: 8 }}>Pick an Item</p>
              <div style={{ position: "relative", marginBottom: 8 }}>
                <Search size={13} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "#7a8473" }} />
                <input
                  value={search}
                  ...
                  style={{
                    ...
                    color: "#1b1d1b", boxShadow: "0 1px 4px rgba(0,0,0,0.04)",
                  }}
                />
              </div>
              <div style={{ ...glass, borderRadius: 16, overflow: "hidden", maxHeight: 200, overflowY: "auto" }}>
                {isLoading
                  ? <p style={{ padding: 14, fontSize: 13, color: "rgba(50,64,54,0.48)" }}>Loading items...</p>
                  : searchFiltered.length === 0
                    ? <p style={{ padding: 14, fontSize: 13, color: "rgba(50,64,54,0.48)" }}>No items found</p>
                    : searchFiltered.map(p => (
                        <button key={p.id} onClick={() => setSelectedId(p.id)}
                          style={{
                            width: "100%", display: "flex", alignItems: "center", gap: 10,
                            padding: "10px 14px", background: selectedId === p.id ? "rgba(31,122,92,0.12)" : "transparent",
                            borderBottom: "1px solid rgba(255,250,241,0.7)", color: selectedId === p.id ? "#1f7a5c" : "rgba(50,64,54,0.7)",
                            border: "none", cursor: "pointer", fontSize: 13, fontWeight: 500,
                            transition: "background 0.12s ease", textAlign: "left",
                          }}
                          onMouseEnter={(e) => { if (selectedId !== p.id) (e.currentTarget as HTMLButtonElement).style.background = "rgba(255,250,241,0.55)"; }}
            ...
            {selectedProduct && (
              <>
                ...
                >
                  {isRunning
                    ? <><motion.div animate={{ rotate: 360 }} transition={{ duration: 0.4, repeat: Infinity, ease: "linear" }}><div style={{ width: 16, height: 16, borderRadius: "50%", border: "2px solid rgba(0,0,0,0.12)", borderTopColor: "#154f3d" }} /></motion.div> Checking...</>
              ...
              {isRunning ? (
                <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <div style={{ position: "relative", width: 56, height: 56 }}>
                    <motion.div style={{ position: "absolute", inset: 0, borderRadius: "50%", border: "2px solid rgba(21,79,61,0.15)" }}
                      animate={{ scale: [1, 1.5, 1], opacity: [0.6, 0, 0.6] }} transition={{ duration: 0.7, repeat: Infinity }} />
                    <div style={{ width: "100%", height: "100%", borderRadius: "50%", border: "3px solid rgba(21,79,61,0.15)", borderTopColor: "#154f3d", animation: "spin 0.4s linear infinite" }} />
                  </div>
                  <p style={{ fontSize: 13, color: "rgba(50,64,54,0.60)" }}>Working it out...</p>
                ...
                <motion.div key="result" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  {(() => {
                    ...
                    return (
                      <div style={{ ...glass, borderRadius: 20, padding: 20, background: rs.bg, border: `1px solid ${rs.border}` }}>
                        ...
                        <p style={{ fontSize: 13, lineHeight: 1.6, color: "rgba(50,64,54,0.7)" }}>{result.recommendation}</p>
                      </div>
                    );
                  })()}
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
                    {[
                      { Icon: Gauge,      label: "Days Till It Finishes", value: result.stockoutDays > 365 ? "365+" : `${result.stockoutDays}d`, color: result.stockoutDays < 14 ? "#a83a26" : "#154f3d" },
                      { Icon: BarChart2,  label: "Too-Much Score",        value: `${result.overstockScore}/100`,  color: result.overstockScore > 60 ? "#a8551f" : "#154f3d" },
                      { Icon: DollarSign, label: result.costDirection === "loss" ? "Money at Risk" : "Money Saved", value: `₹${result.costImpact.toLocaleString()}`, color: result.costDirection === "loss" ? "#a83a26" : "#154f3d" },
                    ].map((s, i) => (
                      <div key={i} style={{ ...glass, borderRadius: 16, padding: "14px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}>
                          <s.Icon size={13} style={{ color: "rgba(50,64,54,0.48)" }} strokeWidth={2} />
                          <span style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: "rgba(50,64,54,0.48)" }}>{s.label}</span>
                        </div>
                  ...
                  <div style={{ ...glass, borderRadius: 20, padding: 20 }}>
                    <p style={{ fontFamily: "var(--font-fraunces), ui-serif, serif", fontSize: 13, fontWeight: 700, color: "#1b1d1b", marginBottom: 14 }}>What This Means</p>
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {result.insights.map((ins, i) => {
                        const InsIcon = ins.type === "warn" ? AlertTriangle : ins.type === "ok" ? CheckCircle2 : Lightbulb;
                        const ic = ins.type === "warn" ? "#a83a26" : ins.type === "ok" ? "#154f3d" : "#154f3d";
                        const ibg = ins.type === "warn" ? "rgba(168,58,38,0.10)" : ins.type === "ok" ? "rgba(21,79,61,0.10)" : "rgba(21,79,61,0.10)";
                        return (
                          <motion.div key={i} initial={{ opacity: 0, x: 6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                            ...
                            <p style={{ fontSize: 13, lineHeight: 1.5, color: "rgba(50,64,54,0.7)" }}>{ins.text}</p>
                          </motion.div>
                        );
                      })}
                    </div>
                  ...
                  {/* Optimization */}
                  <div style={{ ...glass, borderRadius: 20, padding: 20, background: "rgba(200,106,51,0.08)", border: "1px solid rgba(200,106,51,0.18)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                      <Lightbulb size={14} style={{ color: "#a8551f" }} strokeWidth={1.8} />
                      <p style={{ fontFamily: "var(--font-fraunces), ui-serif, serif", fontSize: 13, fontWeight: 700, color: "#1b1d1b" }}>Best Move</p>
                    </div>
                    <p style={{ fontSize: 13, lineHeight: 1.6, color: "rgba(50,64,54,0.7)" }}>{result.optimization}</p>
                  </div>
                </motion.div>
              ) : (
                <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 320, gap: 14 }}>
                  <div style={{ width: 64, height: 64, borderRadius: 20, background: "rgba(21,79,61,0.08)", border: "1px solid rgba(21,79,61,0.18)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <FlaskConical size={26} style={{ color: "#154f3d" }} strokeWidth={1.5} />
                  </div>
                  <h3 style={{ fontFamily: "var(--font-fraunces), ui-serif, serif", fontSize: 15, fontWeight: 700, color: "#1b1d1b", textAlign: "center" }}>Ready to Check</h3>
                  <p style={{ fontSize: 13, color: "rgba(50,64,54,0.60)", textAlign: "center", maxWidth: 280, lineHeight: 1.5 }}>
